...
// function Detail() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            const token = "";
            try {
                const res = await axios.get(`/collects/${id}`, {  // id를 사용하는 URL
                    headers: {
    ...
    return (
        ...
        <div className='Detail-wrapper'>
            <div className='Detail-image'>
                <img src={model ? model.imageUrl : 'default_image.png'} alt={model ? model.title : 'Loading...'} />
            </div>
            ...
            <p style={ { color : '#6F6F6F', textAlign : 'left' }}>👥 {model ? model.capacity : "Loading..."}명</p>
            <p style={ { color : '#214FC7', textAlign : 'right' }}><b>💵 {model ? model.point : "Loading..."} DBC 지급</b></p>
            </div>
            <div className='Detail-condition'>
                ...
                <ul style={{padding: '0px 16px 16px 25px'}}>
                    {/* 조건 내용을 동적으로 렌더링 */}
                    {model && model.requirements.map((requirements) => (
                        <li key={requirements.id} style={{ textAlign: 'left'}}>{requirements.value}</li>
                    ))}
                </ul>
            </div>
            <div> 
